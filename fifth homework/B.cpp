...
struct TSegments {
    ...
    void Update(int64_t vertex, int64_t left, int64_t right, int64_t pos, int64_t value);
};
...
void TSegments::Update(int64_t vertex, int64_t left, int64_t right, int64_t pos, int64_t value) {
    if (left == right) {
        array[vertex] = value;
    } else {
        int64_t mid = (left + right) / 2;
        if (pos <= mid) {
            Update(vertex * 2, left, mid, pos, value);
        } else {
            Update(vertex * 2 + 1, mid + 1, right, pos, value);
        }
        array[vertex] = Sum(array[vertex * 2], array[vertex * 2 + 1]);
    }
}
...
int main() {
    ...
    for (int64_t i = 0; i < m; ++i) {
        ...
        if (symbol == 'u') {
            segments.Update(1, 0, n - 1, num1 - 1, num2);
        }
    }
    delete [] a;
    std::cout << std::endl;
